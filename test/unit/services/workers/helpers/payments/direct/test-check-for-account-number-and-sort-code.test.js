...
describe('services/workers/helpers/payments/direct/check-for-account-number-and-sort-code', () => {
  it(`should return ${false} when there is no missing data`, () => {
    const missingData = checkForAccountNumberAndSortCode(completePaymentData)
    expect(missingData).toEqual(false)
  })

  it(`should return ${true} when there is a missing sort code`, () => {
    const missingData = checkForAccountNumberAndSortCode(missingSortCode)
    expect(missingData).toEqual(true)
  })

  it(`should return ${true} when there is a missing account number`, () => {
