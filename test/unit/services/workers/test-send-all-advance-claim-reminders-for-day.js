...
describe('services/send-all-advance-claim-reminders-for-day', function () {
  ...
  beforeEach(function () {
    ...
    jest.mock('../../../../app/services/data/get-all-open-advance-claims-for-date-of-journey-range-with-email', () => mockGetAllOpenAdvanceClaimsForDateOfJourneyRangeWithEmail)
  ...
  it('should get open advanced claim reminders within one day of DateOfJourney and create notification tasks', function () {
    ...
    return sendAllAdvanceClaimRemindersForDay.sendAllAdvanceClaimRemindersForDay(dateCreated)
      .then(function () {
        expect(mockGetAllOpenAdvanceClaimsForDateOfJourneyRangeWithEmail).toHaveBeenCalledTimes(2) //eslint-disable-line
        expect(moment(mockGetAllOpenAdvanceClaimsForDateOfJourneyRangeWithEmail.mock.calls[0][0]).format('YYYY-MM-DD HH:mm')).toBe(startDateString)
        expect(moment(mockGetAllOpenAdvanceClaimsForDateOfJourneyRangeWithEmail.mock.calls[0][1]).format('YYYY-MM-DD HH:mm')).toBe(endDateString)
