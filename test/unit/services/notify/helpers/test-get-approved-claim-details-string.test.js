...
describe('notify/helpers/get-approved-claim-details-string', () => {
  const claimExpense1 = [
    {
  ...
  it('should contain journey info if claim expense type is a journey type', () => {
  ...
  it('should contain just the expense type if claim expense type is not a journey type', () => {
    const claimDetails = getApprovedClaimDetailsString(claimExpense1, [])
    expect(claimDetails).toContain('Accommodation')
  })

  it('should contain the correct claim amount without deductions', () => {
    const claimDetails = getApprovedClaimDetailsString(claimExpense2, [])
    expect(claimDetails).toContain('Claimed: £30.00')
  })

  it('should contain the correct approved amount without deductions', () => {
    const claimDetails = getApprovedClaimDetailsString(claimExpense2, [])
    expect(claimDetails).toContain('Approved: £20.00')
  })

  it('should contain the deduction header if there are any claim deductions', () => {
    const claimDetails = getApprovedClaimDetailsString(claimExpense1, claimDeduction1)
    expect(claimDetails).toContain('Deductions')
  })

  it('should contain the correct deduction type', () => {
  ...
  it('should contain the correct deduction amount', () => {
    const claimDetails = getApprovedClaimDetailsString(claimExpense2, claimDeduction2)
    expect(claimDetails).toContain('Amount: £10.00')
  })

  it('should return nothing if no claims expenses exist', () => {
