...
describe('services/data/get-number-of-submitted-claims-for-date-range', () => {
  beforeAll(() => {
  ...
  it('should return number of claims submitted', () => {
    return getNumberOfSubmittedClaimsForDateRange(START_DATE, END_DATE).then(submittedClaimCount => {
      expect(submittedClaimCount).toBe(2)
    })
  })

  it('should return zero when no claims', () => {
    const noClaimsStartDate = new Date('1900-01-01')
    const noClaimsEndDate = new Date('1900-01-01 23:59')

    return getNumberOfSubmittedClaimsForDateRange(noClaimsStartDate, noClaimsEndDate).then(submittedClaimCount => {
